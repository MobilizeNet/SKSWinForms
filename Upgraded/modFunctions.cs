...
namespace SKS
{
	internal static class modFunctions
	{
		...
		internal static bool TextBoxNumberEmpty(TextBox textbox)
		{
			...
			{
				result = true;
				MessageBox.Show("The field requires a numeric value.", AssemblyHelper.GetTitle(System.Reflection.Assembly.GetExecutingAssembly()), MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
				textbox.Focus();
				SelectAll(textbox);
			}
			else
